...
const statusLabels = {
  new: 'Нова', accepted: 'Прийнята', rejected: 'Відхилена',
  cancelled: 'Скасована', completed: 'Виконана',
};
...
function DetailContent() {
  ...
  return <SellerPortalLayout><main className="seller-workspace seller-application-detail">
    ...
    <header><div><p className="seller-kicker">Заявка покупця</p><h1>{formatUah(application.amountKopecks)}</h1></div><span className={`seller-detail-status is-${application.status}`}>{statusLabels[application.status]}</span></header>
